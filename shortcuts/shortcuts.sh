...
if command -v gnome-terminal >/dev/null 2>&1; then
    ...
    gsettings set "${TERM_SCHEMA}:${TERM_PATH}" new-tab "'<Primary>t'"
    gsettings set "${TERM_SCHEMA}:${TERM_PATH}" new-window "'<Primary>n'"
    gsettings set "${TERM_SCHEMA}:${TERM_PATH}" close-tab "'<Primary>w'"
    gsettings set "${TERM_SCHEMA}:${TERM_PATH}" close-window "'<Primary><Shift>w'"
    gsettings set "${TERM_SCHEMA}:${TERM_PATH}" find "'<Primary>f'"
    ...
    echo "     in GNOME Terminal, since Paste now intercepts it first. Same for"
    echo "     Ctrl+T, Ctrl+N, Ctrl+W, Ctrl+F — these are normally bash/readline"
    echo "     bindings (transpose-char, next-history, unix-word-rubout,"
    echo "     forward-char) and will now be caught by the terminal first.)"
else
